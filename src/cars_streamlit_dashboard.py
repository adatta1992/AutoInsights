...
st.set_page_config(page_title="Cars Sentiment Dashboard", layout="wide")
st.title("🏀 r/cars Reddit Sentiment Analysis")
st.write("### Reddit Post: Ford Officially Discontinues F-150 Lightning")

# Load the Spacy small english model to assist with nl queries
spacy_mdl = load_spacy_model()
path = Path(__file__).parent.parent / "data" / "cars_subreddit_sentanalysis.csv"
...
query = st.text_input(
    "Got questions?",
    placeholder="Let's find answers! Type here..."
)
